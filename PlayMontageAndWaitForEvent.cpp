...
void UPlayMontageAndWaitForEvent::Activate()
{
	...
	if (GetAbilitySystemComponent())
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = GetAbilitySystemComponent()->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UPlayMontageAndWaitForEvent::OnGameplayEvent));

			if (GetAbilitySystemComponent()->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
		...
		{
			ABILITY_LOG(Warning, TEXT("PlayMontageAndWaitForEvent call to PlayMontage failed!"));
		}
	}
	else
	{
		ABILITY_LOG(Warning, TEXT("PlayMontageAndWaitForEvent called on invalid AbilitySystemComponent"));
	}

	if (!bPlayedMontage)
	{
		ABILITY_LOG(Warning, TEXT("PlayMontageAndWaitForEvent called in Ability %s failed to play montage %s; Task Instance Name %s."), *Ability->GetName(), *GetNameSafe(MontageToPlay), *InstanceName.ToString());
...
void UPlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (GetAbilitySystemComponent())
	{
		GetAbilitySystemComponent()->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
...
UAbilitySystemComponent* UPlayMontageAndWaitForEvent::GetAbilitySystemComponent()
{
...
void UPlayMontageAndWaitForEvent::OnMontageBlendingOut(UAnimMontage * Montage, bool bInterrupted)
{
	if (Ability && Ability->GetCurrentMontage() == MontageToPlay)
	{
		...
		{
			GetAbilitySystemComponent()->ClearAnimatingAbility(Ability);
